...
async function deletarCambio(id) {
    try {
        id = parseInt(id, 10); // Garantindo que id seja um número inteiro
        const sql = "DELETE FROM cambio WHERE id_cambio = ?";
        const resultado = await executarQuery(sql, [id]);
        return resultado;
    } catch (error) {
        throw new AppError('Erro ao deletar câmbio', codigoErro, 'CAMBIO_DELETE_ERROR', error.message);
    }
}

export { deletarCambio }
